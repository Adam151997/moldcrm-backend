...
class Lead(models.Model):
    ...
    notes_text = models.TextField(blank=True)  # CHANGED FROM 'notes' TO 'notes_text'
...
class Contact(models.Model):
    account = models.ForeignKey(Account, on_delete=models.CASCADE)
    lead = models.OneToOneField(Lead, on_delete=models.SET_NULL, null=True, blank=True, related_name='converted_contact')
    first_name = models.CharField(max_length=100)
    last_name = models.CharField(max_length=100)
    email = models.EmailField()
    phone = models.CharField(max_length=20, blank=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['account', 'email']),
            models.Index(fields=['account', 'company']),
            models.Index(fields=['account', 'created_at']),
        ]
        unique_together = ['account', 'email']
...
class Deal(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['account', 'assigned_to']),
            models.Index(fields=['account', 'expected_close_date']),
            models.Index(fields=['account', 'created_at']),
        ]
...
class Note(models.Model):
    ...
    # Links to related objects (at least one required)
    lead = models.ForeignKey(Lead, on_delete=models.CASCADE, null=True, blank=True, related_name='note_entries')  # CHANGED related_name
    contact = models.ForeignKey(Contact, on_delete=models.CASCADE, null=True, blank=True, related_name='note_entries')  # CHANGED related_name
    deal = models.ForeignKey(Deal, on_delete=models.CASCADE, null=True, blank=True, related_name='note_entries')  # CHANGED related_name
    ...
    class Meta:
        db_table = 'crm_note'
        indexes = [
            models.Index(fields=['account', 'lead']),
            models.Index(fields=['account', 'contact']),
            models.Index(fields=['account', 'deal']),
        ]
        ordering = ['-created_at']
